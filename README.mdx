
<div align="center">
  
[![Type 2](https://img.shields.io/static/v1?label=|&message=WEBSITE&color=ff&style=for-the-badge&logo=realm&logo-color=white)](http://kurtisrogers.com)
[![Type 2](https://img.shields.io/static/v1?label=|&message=LINKED-IN&color=cdf998&style=for-the-badge&logo=linkedin&logo-color=white)](https://www.linkedin.com/in/roguetrufflepig/)
[![Type 2](https://img.shields.io/static/v1?label=|&message=RESUME&color=24555f&style=for-the-badge&logo=solid&logo-color=white)](https://docs.google.com/document/d/1gAEWDdSs0BjWWMU-GtNehQqGVLNzoE5h12ZatL2RLdY/edit?usp=sharing)
[![Gmail icon](https://img.shields.io/static/v1?label=|&message=GMAIL&color=24555f&style=for-the-badge&logo=gmail&logo-color=white)](mailto:krogers@gmail.com)
[![Linktree icon](https://img.shields.io/static/v1?label=|&message=LINKTREE&color=24555f&style=for-the-badge&logo=linktree&logo-color=white)](https://linktr.ee/kurtisrogers)
  
# kurtisrogers.com
Website build

![GitHub Actions Workflow Status](https://img.shields.io/github/actions/workflow/status/kurtmcmurt/kurtisrogers.com/node.js.yml)

</div>

Hello and welcome to the repo for my Solid JS website project. I wanted to try out Solid so thought I'd build a new website - *for myself* - using it. REALLY enjoying it so far and highly recommend everyone give it a go!


If you want something a bit more bells and whistles for Solid (like Next or Nuxt), check out [Solid Start](https://docs.solidjs.com/solid-start)!


This project is using the ultra naked quickstart version which you can find [here](https://docs.solidjs.com/quick-start#for-typescript-projects). Enjoy!

> [!IMPORTANT]
> ## Node version
> This project uses nvm so you if you have that set up, you can simply run `nvm use`.
>
> **Recommended**
> ```sh
> v20.18.0`
> ```
>
> ## Scripts
> First, install all packages:
> 
> ```sh
> yarn install
> ```

> [!NOTE]
> ### Production mode (locally)
> ```sh
>   yarn serve # you'll want to run "yarn build" before this step
> ```
>
> ### Development mode (fun times 🎉)
> ```sh
>   yarn dev
> ```
>
> ### Build
> ```sh
> yarn build
> ```
>
> ### Storybook
> ```sh
> yarn storybook
> ```
>
> ### Storybook build
> ```sh
> yarn storybook:build
> ```
> 
> ### Test
> ```sh
> yarn test
> ```
>
> ### Test update
> *For those pesky snapshots!*
>
> ```sh
> yarn test:update
> ```
>
> ### Husky
> *For that precommit magic!*
> ```sh
> yarn postinstall
> ```
> 

>## Environment variables
>TODO

>## API information
>TODO

[![Kurtis' GitHub stats](https://github-readme-stats.vercel.app/api?username=kurtmcmurt)](https://github.com/rahulkarda/readme-components)
